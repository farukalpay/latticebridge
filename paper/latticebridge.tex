...
These settings define a fixed local-compute budget for comparing the four decoding methods. The complete run configuration is stored in \path{results/benchmark_structured_validation/validation_config.json}.

\Needspace{22\baselineskip}
\section{Training Dynamics and Operating Frontier}
Table~\ref{tab:training-loss} reports the optimization trace for the prefix model, and Figure~\ref{fig:training-curve} plots the same losses. The loss trajectory is monotone over the reported epochs and supports the use of a fixed shared checkpoint for all decoding comparisons.
